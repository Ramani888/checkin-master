...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Image, ScrollView, ActivityIndicator, Modal, Alert, Linking, Platform, ToastAndroid } from 'react-native';
...
const Setting: React.FC = () => {
    ...
    const handleConnect = async () => {
        try {
            ...
            const isConnected = await checkInternetConnection();
            if (!isConnected) {
                setMessage(strings.noInternetConnection);
                return;
            }
            ...
            if (tokenData?.access_token) {
                ...
                await Promise.all([
                    StorageHelper.saveItem('endPointUrl', endPointUrl),
                    StorageHelper.saveItem('clientId', clientId),
                    StorageHelper.saveItem('clientSecret', clientSecret),
                    StorageHelper.saveItem('token', access_token),
                    StorageHelper.saveItem('expiryTime', expiryTime.toString())
                ]);
            ...
            logError('Failed to connect', error);
            showToast(strings.failedToConnect);
        } finally {
            setShowLoadingDialog(false);
            checkToken();
        }
